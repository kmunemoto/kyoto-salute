-- 1) Triple session base EXP in process_session_rewards
...
-- 2) Award 2 gacha tickets per visit
ALTER TABLE public.user_gacha_tickets
  ADD COLUMN IF NOT EXISTS ticket_seq SMALLINT NOT NULL DEFAULT 1;

DROP INDEX IF EXISTS public.user_gacha_tickets_user_session_date_uniq;

CREATE UNIQUE INDEX IF NOT EXISTS user_gacha_tickets_user_session_seq_uniq
  ON public.user_gacha_tickets (user_id, session_date, ticket_seq)
...
BEGIN
  BEGIN
    INSERT INTO public.user_gacha_tickets (user_id, session_date, ticket_seq)
    VALUES (NEW.user_id, NEW.workout_date, 1),
           (NEW.user_id, NEW.workout_date, 2)
    ON CONFLICT (user_id, session_date, ticket_seq) WHERE session_date IS NOT NULL DO NOTHING;
