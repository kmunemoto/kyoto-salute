...
const TrainerView = () => {
  return (
    <div className="min-h-screen bg-background pt-14 pb-8 px-4 md:px-8 max-w-6xl mx-auto fade-in">
      {/* Header */}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-muted/40">
                  <th className="text-left text-xs font-bold text-muted-foreground p-3 uppercase tracking-wider">名前</th>
                  <th className="text-left text-xs font-bold text-muted-foreground p-3 uppercase tracking-wider">目標</th>
                  <th className="text-left text-xs font-bold text-muted-foreground p-3 uppercase tracking-wider">次回予約</th>
                  <th className="text-left text-xs font-bold text-muted-foreground p-3 uppercase tracking-wider">回数</th>
                  <th className="text-left text-xs font-bold text-muted-foreground p-3 uppercase tracking-wider min-w-[140px]">進捗</th>
                </tr>
              </thead>
              <tbody>
                {clients.map((c) => (
                  <tr key={c.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors cursor-pointer">
                    <td className="p-3">
                    ...
                    <td className="p-3 text-sm font-medium">{c.nextSession}</td>
                    <td className="p-3 text-sm font-medium">{c.totalSessions}回</td>
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <Progress value={c.progress} className="h-2 flex-1" />
                        <span className="text-xs font-bold text-muted-foreground w-8">{c.progress}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
